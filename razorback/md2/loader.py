...
class Loader( object ):
    ...
    def read_header( f ):
        ...
        header = Loader.header_layout._make(
            Loader._load_block( f, '< 4s16l', 1 )[ 0 ]
            )

        if header.ident != Loader.id:
            raise ValueError(
        ...
        if header.version != Loader.version:
            raise ValueError(
    ...
    def read_texture_coordinates( f, header ):
        ...
        tcs = numpy.array(
            Loader._load_block( f, '< 2h', header.num_st ),
            dtype = numpy.float
            )
        tcs.shape = (-1, 2)
    ...
    def read_triangles( f, header ):
        ...
        triangles = numpy.array(
            Loader._load_block( f, '< 6H', header.num_tris ),
            dtype = numpy.uint16
            )
        triangles.shape = (-1, 6)
        ...
        triangles = Loader.triangle_layout._make(
    ...
    def read_frames( f, header ):
        ...
        return [ Loader.read_frame( f, header ) for x in xrange( header.num_frames ) ]
    ...
    def read_frame( f, header ):
        ...
        name, = Loader._load_block( f, '< 16s', 1 )[0]
        ...
        frame_vertex_data = numpy.array(
            Loader._load_block( f, '<4B', header.num_vertices ),
            dtype = numpy.uint8
            )
        ...
        normals = Loader.normal_lookup_table[ normal_indices ]
        normals.shape = (-1, 3)

        return Loader.frame_layout._make(
    ...
    def convert_indicies_for_all_frames( frames, triangles ):
        ...
        """
        return [ Loader.convert_indices_for_frame( frame, triangles ) for frame in frames ]
